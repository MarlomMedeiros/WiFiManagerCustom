...
void WiFiManager::WiFiManagerInit()
{
    ...
    if (_debug && _debugLevel >= WM_DEBUG_DEV)
        debugPlatformInfo();
    _max_params = WIFI_MANAGER_MAX_PARAMS;
}
...
#ifdef WM_DEBUG_LEVEL
    if (!res)
        DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] hostname: set failed!"));
#endif
...
#ifdef WM_DEBUG_LEVEL
    if (!ret)
        DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] There was a problem starting the AP"));
...
// set ap hostname
    ...
    if (ret && _hostname != "")
    {
        ...
        if (!res)
            DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] hostname: AP set failed!"));
...
{
    // Reserve capacity to avoid memory fragmentation on ESP32-C3
    std::vector<std::pair<int, int>> rssiList;
    rssiList.reserve(n > 0 ? n : 0);
    ...
    // Limit to max 8 networks to prevent device overload (optimized for ESP32-C3)
    const int WM_MAX_NETWORKS = 5;
    // Minimum RSSI threshold to filter weak signals (-75 dBm ~ 25% quality)
    const int MIN_RSSI_THRESHOLD = -75;
    int displayCount = 0;
    ...
    else if (n)
    {
        ...
        for (const auto &pair : rssiList)
        {
            ...
            // Skip networks with weak signal (optimization for areas with many networks)
        ...
        // Note: WiFi.scanDelete() is called by the handler after this function
    }
    wifiList += "]";
}

// Rota para escanear redes Wi-Fi (optimized for ESP32-C3 with many networks)
void WiFiManager::handleScanWiFi()
{
    ...
    if (n == WIFI_SCAN_FAILED)
    {
        ...
        server->send(200, "application/json", "[]"); // Return empty, client should retry
        return;
    }
    else if (n == WIFI_SCAN_RUNNING)
    {
        // Scan in progress, tell client to wait
        server->send(200, "application/json", "[]"); // Return empty, client should retry
        return;
    }
    ...
    printScanResult(n);
    WiFi.scanDelete(); // Free memory after processing
...
#ifdef WM_DEBUG_LEVEL
    if (!ret)
        DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] disconnect configportal - softAPdisconnect FAILED"));
...
#ifdef WM_DEBUG_LEVEL
        if (!ret)
            DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] wifi config failed"));
        else
...
uint8_t WiFiManager::waitForConnectResult()
{
    ...
    if (_connectTimeout > 0)
        DEBUG_WM(WM_DEBUG_DEV, _connectTimeout, F("ms connectTimeout set"));
#endif
...
boolean WiFiManager::captivePortal()
{
    ...
    if (!_enableCaptivePortal || !configPortalActive)
        return false; // skip redirections if cp not enabled or not in ap mode
...
void WiFiManager::setDebugOutput(boolean debug)
{
    ...
    if (_debug && _debugLevel == WM_DEBUG_DEV)
        debugPlatformInfo();
    if (_debug && _debugLevel >= WM_DEBUG_NOTIFY)
        DEBUG_WM((__FlashStringHelper *)WM_VERSION_STR, " D:" + String(_debugLevel));
}
...
// ret = esp_wifi_set_bandwidth(WIFI_IF_AP,WIFI_BW_HT20); // WIFI_BW_HT40
    ...
    if (err)
    {
        if (err == ESP_ERR_WIFI_NOT_INIT)
            DEBUG_WM(WM_DEBUG_ERROR, "[ERROR] ESP_ERR_WIFI_NOT_INIT");
        else if (err == ESP_ERR_INVALID_ARG)
            DEBUG_WM(WM_DEBUG_ERROR, "[ERROR] ESP_ERR_WIFI_ARG (invalid country code)");
        else if (err != ESP_OK)
            DEBUG_WM(WM_DEBUG_ERROR, "[ERROR] unknown error", (String)err);
    }
#endif
    ret = err == ESP_OK;
    ...
    if (_wificountry == "US")
        ...
        DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] country code not found"));
#endif
#endif

#ifdef WM_DEBUG_LEVEL
    if (ret)
        DEBUG_WM(WM_DEBUG_VERBOSE, F("[OK] esp_wifi_set_country: "), _wificountry);
    else
        DEBUG_WM(WM_DEBUG_ERROR, F("[ERROR] esp_wifi_set_country failed"));
#endif
    return ret;
}
...
#ifdef WM_DEBUG_LEVEL
            if (persistent)
                DEBUG_WM(WM_DEBUG_DEV, F("enableSTA PERSISTENT ON"));
#endif
